    ...
    window.afm_tracker = {
      ...
      load: function () {
        ...
        if (this.linkId == null) {
          ...
          } //new user, no campaign.
          else this.sc('afm.link_id', '-1', afm_server_info.keep_days);
        } //prolonging current owner.
        else {
          this.sc('afm.link_id', this.linkId, afm_server_info.keep_days);
      ...
      genId: function () {
        var id = [];
        for (var i = 0; i < 24; i++)
          id.push('0123456789abcdefghijklmnopqrstuvwxyz'[Math.floor(Math.random() * 36)]);
        return id.join('');
      },
      ...
      log: function (event) {
        ...
        navigator.sendBeacon(
          `${afm_server_info.ajax_url}?action=afm_log&security=${
            afm_server_info.nonce
      ...
      tld: function () {
        if ((document.domain.match(/\./g) || []).length < 2) return '.' + document.domain;
        else if (
          document.domain.match(
            /(?:[\w-]+\.){1}(?:com|net|org|co|ac|gov|edu|mil|info|nom|firm|gen|ind|idv|me)?(?:\.[a-z]{2})?$/i
          )
        )
          return (
            '.' +
            /(?:[\w-]+\.){1}(?:com|net|org|co|ac|gov|edu|mil|info|nom|firm|gen|ind|idv|me)?(?:\.[a-z]{2})?$/i.exec(
              document.domain
            )[0]
          );
        else if (document.domain.match(/.+?(\.[\w-]+\.[a-z]{2,4})$/i))
          return /.+?(\.[\w-]+\.[a-z]{2,4})$/i.exec(document.domain)[1];
        else return document.domain;
      },
      ...
      report: function (event) {
        if (
          typeof window.afm_server_info != 'object' ||
          typeof window.afm_server_info.aff_pixel != 'string'
        )
          return;
